...
toggle_audio() {
    ...
    family_id=$(wpctl status | grep -A20 "Audio" | grep -A20 "Sinks:" | grep "Family.*HD Audio" | grep -oE '[0-9]+' | head -n1)
    kraken_id=$(wpctl status | grep -A20 "Audio" | grep -A20 "Sinks:" | grep "Kraken.*Tournament" | grep -oE '[0-9]+' | head -n1)
